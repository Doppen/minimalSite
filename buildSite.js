...
function createSite() {
  generateHtml()
  createSass('src/scss/ineo-ui.scss')
  createSass('src/scss/util.scss')
  createSass('src/scss/lamachine-style.scss')
}
          ...
          list.forEach(function(file) {
            ...
            fs.stat(file, function(err, stat) {
              ...
              if (stat && stat.isDirectory()) {
                ...
                fs.readFile('src/components/'+file, 'utf-8', function(error, source){
...
function createSass(pathFile) {
  ...
  sass.render({
    ...
    if (err) {
      ...
      let filename = path.basename(pathFile).replace(path.extname(pathFile), '')
      createFile(outputDir+'css/v'+outputVersion+'/'+filename+'.css', result.css.toString())
    }
  });
}
